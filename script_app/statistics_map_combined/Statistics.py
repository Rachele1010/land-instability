...
def Statistics_Data(df_list, filenames):
    ...
    # Sezione per Merge Datasets (UNICO GRAFICO)
        ...
        for dataset in filenames:
            if st.checkbox(f"Select {dataset}", value=dataset in st.session_state.selected_distribution_datasets):
                selected_datasets.append(dataset)
                # Aggiungi il dataset selezionato al session_state per memorizzarlo
                if dataset not in st.session_state.selected_distribution_datasets:
                    st.session_state.selected_distribution_datasets.append(dataset)
            else:
                # Rimuovi il dataset da session_state se non è selezionato
                if dataset in st.session_state.selected_distribution_datasets:
                    st.session_state.selected_distribution_datasets.remove(dataset)
        ...
        if selected_datasets:
            for dataset_name in selected_datasets:
                ...
                # Visualizza il nome del dataset
